...
public class MemberServiceImpl implements MemberDataService {
	...
	@Override
	public List<MemberResponseDTO> getMemberList(MemberFilterDTO filterDTO){
		...
		return list.stream().map(member-> mapper.map(member, MemberResponseDTO.class)).toList();
	}
	...
	public MemberResponseDTO getMember(UUID memberId) {
		...
		return memberRepository.findById(memberId)
				.map(member-> mapper.map(member, MemberResponseDTO.class))
