...
import { GetPovertyTrends200 } from '@core/model/analyticsService';
...
@Component({
  selector: 'app-analytics-family-poverty-line-chart',
  standalone: true,
  imports: [
    RouterModule,
    MatCardModule,
...
export class AnalyticsFamilyPovertyLineChartComponent {
  ...
  trends = toSignal<GetPovertyTrends200>(
    this.authService.currentUser.pipe(
      map((user) => user?.id),
      switchMap((id) => {
        if (!id) return of({});

        return this.analyticsService.getPovertyTrends({
          analyticsFilterDTO: {
