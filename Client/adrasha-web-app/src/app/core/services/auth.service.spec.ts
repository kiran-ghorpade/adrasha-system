...
import { AuthService, LoginService, TokenService } from '@core/services';
...
describe('AuthService', () => {
  ...
  beforeEach(() => {
    ...
    authService.change().subscribe((user: User) => {
  ...
  it('should log in failed', () => {
    authService
      ...
      .subscribe((isLogin: boolean) => expect(isLogin).toBeFalse());
  ...
  it('should log in successful and get user info', () => {
    ...
    authService
      .login(email, 'password', false)
      .subscribe((isLogin: boolean) => expect(isLogin).toBeTrue());
