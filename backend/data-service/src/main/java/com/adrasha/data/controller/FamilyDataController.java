...
public class FamilyDataController {
	...
	@GetMapping
	public Page<FamilyResponseDTO> getAllFamilies(
		...
		Page<FamilyResponseDTO> dto = familyPages.map(family -> mapper.map(family, FamilyResponseDTO.class));
	...
	public FamilyResponseDTO getFamily(@PathVariable UUID id){
		...
		return mapper.map(request, FamilyResponseDTO.class);
		
	}
	
	@PostMapping
	public ResponseEntity<FamilyResponseDTO> createFamily(@Valid @RequestBody FamilyRegistrationDTO familyRegistrationDTO){
	...
	@PutMapping("/{id}")
	public FamilyResponseDTO udpateFamily(
